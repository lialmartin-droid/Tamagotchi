...
function phase(age){
 const chickStart=HATCH_TIME;
 const pulletStart=chickStart+CHICK_TIME;
 const henStart=pulletStart+PULLET_TIME;
 const matronStart=henStart+HEN_TIME;
 if(age<chickStart)return {id:'egg',name:'Vajíčko v hnízdě',start:0,next:chickStart,place:'hnízdo'};
 if(age<pulletStart)return {id:'chick',name:'Kuřátko pod lampou',start:chickStart,next:pulletStart,place:'výhřevná lampa'};
 if(age<henStart)return {id:'pullet',name:'Mladá slepička ve výběhu',start:pulletStart,next:henStart,place:'výběh'};
 if(age<matronStart)return {id:'hen',name:'Dospělá slepice',start:henStart,next:matronStart,place:'bidlo'};
 return {id:'matron',name:'Matka hejna',start:matronStart,next:null,place:'hlavní bidlo'};
}
...
function migrateState(){
  if(!state || typeof state!=='object'){state=null;return false;}
  const t=now();
  state.name=typeof state.name==='string'&&state.name.trim()?state.name.trim().slice(0,40):'Kokoška';
  state.created=Number.isFinite(Number(state.created))?Number(state.created):t;
  state.last=Number.isFinite(Number(state.last))?Math.min(Number(state.last),t):t;
  ['hunger','thirst','health','happy','warmth'].forEach(k=>{state[k]=clamp(Number.isFinite(Number(state[k]))?Number(state[k]):(k==='health'?100:90));});
  const savedColor=state.color&&typeof state.color==='object'?state.color:null;
  const matchedColor=savedColor&&colors.find(c=>c.name===savedColor.name);
  state.color=matchedColor||colors[0];
  state.trait=traits.some(t=>t.id===state.trait)?state.trait:traits[Math.floor(Math.random()*traits.length)].id;
  state.eggs=Math.max(0,Math.floor(Number(state.eggs)||0));
  state.eggHistory=Array.isArray(state.eggHistory)?state.eggHistory.slice(0,100):[];
  const ai=state.ai&&typeof state.ai==='object'?state.ai:{};
  state.ai={
    x:Number.isFinite(Number(ai.x))?Math.max(22,Math.min(90,Number(ai.x))):50,
    y:Number.isFinite(Number(ai.y))?Math.max(0,Math.min(60,Number(ai.y))):36,
    action:typeof ai.action==='string'?ai.action.slice(0,80):'odpočívá',
    nextMove:Number.isFinite(Number(ai.nextMove))?Number(ai.nextMove):0,
    prevX:Number.isFinite(Number(ai.prevX))?Number(ai.prevX):50
  };
  const defaults={worms:0,grain:0,slugs:0,berries:0,stones:0,scratches:0,miniGames:0,sleepHours:0,outsideHours:0};
  state.stats=Object.assign(defaults,state.stats&&typeof state.stats==='object'?state.stats:{});
  Object.keys(defaults).forEach(k=>{state.stats[k]=Math.max(0,Number(state.stats[k])||0);});
  ...
  state.log=Array.isArray(state.log)?state.log.slice(0,80).map(x=>String(x).slice(0,300)):['Načtena starší uložená hra.'];
  state.nextEggAt=Number.isFinite(Number(state.nextEggAt))?Number(state.nextEggAt):null;
  state.weatherAt=Number.isFinite(Number(state.weatherAt))?Number(state.weatherAt):0;
  if(!state.weather||typeof state.weather!=='object')state.weather=null;
  return true;
}
...
async function fetchWeather(){
  ...
  try{
    ...
    if(!data||!data.current||!Number.isFinite(Number(data.current.weather_code)))throw new Error('Neplatná data počasí');
    state.weather={temp:Number.isFinite(Number(data.current.temperature_2m))?Number(data.current.temperature_2m):null,code:Number(data.current.weather_code),text:weatherCode(Number(data.current.weather_code))};
    state.weatherAt=now();
    save();
    render();
  }catch(e){
    if(!state.weather)state.weather={temp:null,code:null,text:'offline'};
    render();
  }
}
function render(){const age=now()-state.created;const ph=phase(age);const part=dayPart();updateAi(ph,part);document.body.className=((part==='night'?'sky-night':part==='dawn'?'sky-dawn':part==='evening'?'sky-evening':'')+' '+weatherClass()+(isRainy()?' rainy':'')).trim();document.getElementById('clock').textContent=new Date().toLocaleTimeString('cs-CZ',{hour:'2-digit',minute:'2-digit'});document.getElementById('petName').textContent=state.name+' ('+state.color.name+')';document.getElementById('phaseName').textContent=ph.name;document.getElementById('ageText').textContent=ageText(age);const sun=getSunTimes();document.getElementById('sunText').textContent=sun.rise.toLocaleTimeString('cs-CZ',{hour:'2-digit',minute:'2-digit'})+' / '+sun.set.toLocaleTimeString('cs-CZ',{hour:'2-digit',minute:'2-digit'});document.getElementById('weatherText').textContent=state.weather?(state.weather.text+(state.weather.temp!=null?' '+Math.round(state.weather.temp)+' °C':'')):'offline';document.getElementById('traitText').textContent=getTrait().name;document.getElementById('activityText').textContent=state.ai.action;document.getElementById('eggsText').textContent=state.eggs+' ks';document.getElementById('eggCounter').textContent='🥚 '+state.eggs;['hunger','thirst','health','happy'].forEach(k=>{document.getElementById(k+'Num').textContent=state[k]+' %';document.getElementById(k+'Bar').style.width=state[k]+'%'});
const pct=ph.next?Math.max(0,Math.min(100,Math.round((age-ph.start)/(ph.next-ph.start)*100))):100;document.getElementById('lifeBadge').textContent=(ph.id==='egg'?'🥚 ':ph.id==='chick'?'🐣 ':ph.id==='pullet'?'🐥 ':ph.id==='hen'?'🐔 ':'👑 ')+pct+' %';
...
function renderLog(){
  ...
  if(!box)return;
  box.replaceChildren(...state.log.map(x=>{const d=document.createElement('div');d.textContent=x;return d;}));
}
...
function exportSave(){
  ...
  a.href=url;a.download='slepicka-zaloha-'+new Date().toISOString().slice(0,10)+'.json';a.click();
  setTimeout(()=>URL.revokeObjectURL(url),0);
}
function importSave(e){
  const file=e.target.files[0];if(!file)return;
  const r=new FileReader();
  r.onload=()=>{try{const parsed=JSON.parse(r.result);state=parsed;if(!migrateState())throw new Error('Neplatná záloha');save();load();log('Záloha byla načtena.');}catch(err){state=null;alert('Soubor nejde načíst nebo není platnou zálohou hry.');}finally{e.target.value='';}};
  r.readAsText(file);
}
setInterval(()=>tick(false),60000);
setInterval(()=>{if(state)render();},3000);
setInterval(()=>{if(state&&(!state.weatherAt||now()-state.weatherAt>=WEATHER_REFRESH))fetchWeather();},60000);
load();
...
function render(){
  ...
  document.getElementById('activityText').textContent=state.ai.action;
  ...
  const pct=ph.next?Math.max(0,Math.min(100,Math.round((age-ph.start)/(ph.next-ph.start)*100))):100;
  ...
  const fx=document.getElementById('weatherEffects');
  if(fx)fx.classList.toggle('rain',rainOn);
...
/* ==============================================================
   0.6.2 – automatická roční období
   ============================================================== */
...
/* Testovací přepínač v konzoli:
   demoSeason('autumn')
   setDemoWeather('déšť')
*/
window.demoSeason=function(season){
  const allowed=['spring','summer','autumn','winter'];
  if(!allowed.includes(season))return;
  document.body.classList.remove('season-spring','season-summer','season-autumn','season-winter');
